...
This maintainer utility intentionally requires an explicit local SMEDC
...
def resolve_service_repo(args: argparse.Namespace) -> Path:
    ...
    if not registry_path.exists() or not query_path.exists():
        raise RegistryFixtureError(f"not an SMEDC service repo with structured registry sources: {service_repo}")
    return service_repo
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to the SMEDC service repository. May also be supplied via SME_DATA_CENTER_REPO.",
    )
